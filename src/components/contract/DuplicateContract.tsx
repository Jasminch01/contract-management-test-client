...
const EditableContract: React.FC<ContractProps> = ({
  ...
  const handleSave = () => {
    try {
      // Create a new contract object with a new ID
      const newContract = {
        ...contract,
        id: generateNewId(), // You'll need to implement this function
        contractNumber: generateNewContractNumber(), // And this one
        // Clear any existing invoice references if needed
        invoiceId: undefined,
        // Add any other fields that should be reset for a new contract
      };
  ...
  const generateNewId = () => {
    // Example: generate a new unique ID
    return Math.max(...contracts.map((c) => c.id), 0) + 1;
  };

  const generateNewContractNumber = () => {
    // Example: increment the last contract number
    const lastNumber = contracts.reduce((max, c) => {
      const num = parseInt(c.contractNumber?.match(/\d+/)?.toString() || "0");
      return num > max ? num : max;
    }, 0);
    return `CON-${lastNumber + 1}`;
  };
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div>
                    <label className="block text-xs font-medium text-gray-700 uppercase mb-1">
                      Seller Contract Attachment
                    </label>
                    <div className="flex items-center gap-2">
                      <input
                        type="file"
                        accept="application/pdf"
                        onChange={(e) => handleFileUpload(e, "sellersContract")}
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        type="file"
                        accept="application/pdf"
                        onChange={(e) => handleFileUpload(e, "buyersContract")}
                        className="block w-full text-sm text-gray-500
                file:mr-4 file:py-2 file:px-4
                file:rounded-md file:border-0
                file:text-sm file:font-semibold
